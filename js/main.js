const app = {
	build: (a, b = d => void(d), c = undefined) => {
		...
		if (a.elements) a.elements.forEach(e => (typeof e == 'object') ? (nav = nav.concat('<li class="navnav' + (oc == 0 ? ' current' : '') + '" id="' + (e.name || '') + '"><a onclick="divShow(\'' + (e.name || '') + '\')">' + (e.name || '') + '</a></li>'), container = container.concat('<div id="' + (e.name || '') + '" class="element' + (oc == 0 ? '' : ' dispf') + '">' + (r => r.map(d => '<img data-src="' + d.replace(/(\/thumbnail\/)/g, '/') + '" src="' + d + '" class="lazyload" />').join('\n'))(e.images || []) + '<p><span class="center info">' + (e.description || '') + '</span></p></div>'), oc++) : (nav = nav.concat(e)));
